...
##作図
    ...
    ax.set_extent([125,150,33,48])
#    contour=ax.contour(lons,lats,slp,levels=np.arange(940,1040,4),colors="black",linewidths=1.0,transform=ccrs.PlateCarree())
#陸面マスク
    contour=ax.contour(lons,lats,slp_sea,levels=np.arange(940,1040,4),colors="black",linewidths=2.0,transform=ccrs.PlateCarree())
    ax.clabel(contour,fontsize=18)

#気温   
#    shade=ax.contourf(lons,lats,tmp2m,levels=np.arange(-35,45,10),cmap="bwr",transform=ccrs.PlateCarree())
#    plt.colorbar(shade,orientation="horizontal",label="[ °C ]")
...
#風
    ...
    plt.savefig(f"{fig_dir}/n{member}/{experiment}_SLP_V-{time_str}n{member}.png")
    plt.close("all")
...
keyword=f"{experiment}_SLP_V"

files=sorted(glob.glob(f"{fig_dir}/n{member}/{keyword}*.png"))
pprint.pprint(files)

images=list(map(lambda file : Image.open(file),files))
images[0].save(f"{fig_dir}/n{member}/{experiment}n{member}_animation_{keyword}.gif",save_all=True,append_images=images[1:],optimize=True,duration=1000,loop=0)
